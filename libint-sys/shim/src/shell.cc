...
// Shell
// -----------------------------------------------------
std::unique_ptr<Shell> shell() { return std::make_unique<Shell>(); }
...
rust::Vec<double> max_ln_coeff(const Shell &shell) {
  rust::Vec<double> v;
  std::copy(shell.max_ln_coeff.begin(), shell.max_ln_coeff.end(),
            std::back_inserter(v));
  return v;
}
...
std::size_t size_shell(const Shell &shell) { return shell.size(); }
std::size_t ncontr(const Shell &shell) { return shell.ncontr(); }
std::size_t nprim(const Shell &shell) { return shell.nprim(); }
...
int l(const Contraction &contraction) { return contraction.l; }

bool pure(const Contraction &contraction) { return contraction.pure; }

rust::Vec<double> coeff(const Contraction &contraction) {
  rust::Vec<double> v;
  std::copy(contraction.coeff.begin(), contraction.coeff.end(),
            std::back_inserter(v));
  return v;
}
...
size_t size(const Contraction &contraction) { return contraction.size(); }
} // namespace libint2
